...
def decode_redis_hash(h) -> Dict[str, Any]:
    ...
    """
    return dict((as_str(k), h[k]) for k in h)
...
def utcparse(date_str: Optional[Union[str, bytes]]) -> Optional[dt.datetime]:
    if not date_str:
        return None
    try:
        return dt.datetime.fromisoformat(as_str(date_str)).replace(tzinfo=None)
    except ValueError:
        return dt.datetime.strptime(as_str(date_str), '%Y-%m-%dT%H:%M:%SZ')
...
def get_call_string(
    ...
    arg_list = [as_str(truncate_long_string(repr(arg), max_length)) for arg in args]

    list_kwargs = ['{0}={1}'.format(k, as_str(truncate_long_string(repr(v), max_length))) for k, v in kwargs.items()]
