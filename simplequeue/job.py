...
class Job:
    ...
    def get_status(self, refresh: bool = True) -> Optional[JobStatus]:
        ...
        if refresh:
            ...
            self._status = JobStatus(as_str(status))
        return self._status
    ...
    def dependent_ids(self) -> List[str]:
        ...
        return list(map(as_str, self.connection.smembers(self.dependents_key)))
    ...
    def restore(self, raw_data) -> Any:
        ...
        self.created_at = utcparse(obj.get('created_at'))  # type: ignore[assignment]
        self.origin = as_str(obj['origin']) if obj.get('origin') else ''
        self.worker_name = obj['worker_name'].decode() if obj.get('worker_name') else None
        self.description = as_str(obj['description']) if obj.get('description') else None
        self.enqueued_at = utcparse(obj.get('enqueued_at'))
        self.started_at = utcparse(obj.get('started_at'))
        self.ended_at = utcparse(obj.get('ended_at'))
        self.last_heartbeat = utcparse(obj.get('last_heartbeat'))
        self.group_id = as_str(obj['group_id']) if obj.get('group_id') else None
        ...
        self._status = JobStatus(as_str(obj['status'])) if obj.get('status') else None
        ...
        if raw_exc_info:
            try:
                self._exc_info = as_str(zlib.decompress(raw_exc_info))
            except zlib.error:
                # Fallback to uncompressed string
                self._exc_info = as_str(raw_exc_info)
    ...
    def _handle_success(self, result_ttl: int, pipeline: 'Pipeline'):
        ...
        self.save(pipeline=pipeline, include_meta=False, include_result=True)
