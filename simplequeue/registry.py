...
from .utils import as_str, backend_class, current_timestamp
...
class BaseRegistry:
    ...
    def get_expired_job_ids(self, timestamp: Optional[float] = None):
        ...
        return [as_str(job_id) for job_id in expired_jobs]
    ...
    def get_job_ids(self, start: int = 0, end: int = -1):
        ...
        """
        self.cleanup()
        return [as_str(job_id) for job_id in self.connection.zrange(self.key, start, end)]
...
class StartedJobRegistry(BaseRegistry):
    ...
    def cleanup(self, timestamp: Optional[float] = None, exception_handlers: Optional[list] = None):
        ...
        :param timestamp: The timestamp to use as the limit.
        :param exception_handlers: A list of exception handlers to call.
        """
        score = timestamp if timestamp is not None else current_timestamp()
    ...
    def add_execution(self, execution: 'Execution', pipeline: 'Pipeline', ttl=0, force_update: bool = False) -> int:
        ...
        :param force_update: whether the execution update should be forced
        :return: The ZADD command result
        """
        score = ttl if ttl < 0 else current_timestamp() + ttl
        if score == -1:
            score = '+inf'

        return pipeline.zadd(self.key, {execution.composite_key: score}, xx=force_update)  # type: ignore

    def remove_execution(self, execution: 'Execution', pipeline: 'Pipeline'):
        """Removes execution from the registry
...
class FinishedJobRegistry(BaseRegistry):
    ...
    def cleanup(self, timestamp: Optional[float] = None) -> None:
