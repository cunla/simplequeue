...
from .utils import as_str, current_timestamp, now
...
class Execution:
    ...
    def create(cls, job: Job, ttl: int, pipeline: 'Pipeline') -> 'Execution':
        ...
        job.started_job_registry.add_execution(execution, pipeline=pipeline, ttl=ttl, force_update=False)
        return execution
    ...
    def heartbeat(self, started_job_registry: StartedJobRegistry, ttl: int, pipeline: 'Pipeline'):
        ...
        started_job_registry.add_execution(self, ttl=ttl, pipeline=pipeline, force_update=True)
