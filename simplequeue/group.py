...
class Group:
    ...
    def cleanup(self, pipeline: Optional['Pipeline'] = None):
        ...
        job_ids = [as_str(job) for job in list(self.connection.smembers(self.key))]
        expired_job_ids = []
        for job in job_ids:
    ...
    def get_jobs(self) -> list:
        ...
        self.cleanup()
        job_ids = [as_str(job) for job in self.connection.smembers(self.key)]
    ...
    def all(cls, connection: 'Redis') -> List['Group']:
        ...
        group_keys = [as_str(key) for key in connection.smembers(cls.REDIS_GROUP_KEY)]
