...
from .utils import as_str, backend_class, compact, import_attribute, now, parse_timeout
...
class Queue:
    ...
    _QUEUES_REGISTRY: Optional[SetRegistry] = None
    ...
    def all(
        ...
        :param job_class: Job class. Defaults to None.
        :param serializer: Serializer. Defaults to None.
        :param death_penalty_class: Death penalty class. Defaults to None.
        ...
        all_registered_queues = Queue._QUEUES_REGISTRY.all()
    ...
    def __init__(
        ...
        if Queue._QUEUES_REGISTRY is None:
            Queue._QUEUES_REGISTRY = SetRegistry(connection=connection, registry_key='rq:queues')
    ...
    def delete(self, delete_jobs: bool = True):
        ...
        with self.connection.pipeline() as pipeline:
            Queue._QUEUES_REGISTRY.remove(self._key, pipeline=pipeline)
    ...
    def get_job_ids(self, offset: int = 0, length: int = -1) -> List[str]:
        ...
        job_ids = [as_str(job_id) for job_id in self.connection.lrange(self.key, start, end)]
    ...
    def compact(self):
        ...
        while True:
            ...
            if job_id is None:
                break
            if self.job_class.exists(as_str(job_id), self.connection):
    ...
    def schedule_job(self, job: 'Job', datetime: datetime, pipeline: Optional['Pipeline'] = None):
        ...
        # Add Queue key set
        Queue._QUEUES_REGISTRY.add(self.key, pipe)
        job.save(pipeline=pipe)
    ...
    def enqueue_dependents(
        ...
        while True:
            try:
                ...
                dependent_job_ids = {as_str(_id) for _id in pipe.smembers(dependents_key)}  # type: ignore[attr-defined]
    ...
    def pop_job_id(self) -> Optional[str]:
        ...
        Returns:
            job_id (str): The job id
        """
        return as_str(self.connection.lpop(self.key))
    ...
    def dequeue_any(
        ...
        while True:
            ...
            if result is None:
                return None
            queue_key, job_id = map(as_str, result)
