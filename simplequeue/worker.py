...
from typing import TYPE_CHECKING, Callable, List, Optional, Tuple, Type, Union, Dict
from uuid import uuid4

from .model import SetRegistry
...
_KEY_QUEUE_WORKERS = 'rq:queue:{0}:workers'
_KEY_ALL_WORKERS = 'rq:workers'
_QUEUE_WORKERS_REGISTRIES: Dict[str, SetRegistry] = dict()


def clean_queue_worker_registry(queue: 'Queue') -> None:
    """Delete invalid worker keys in registry.
    :param queue: Queue
    """
    registry = _QUEUE_WORKERS_REGISTRIES.get(queue.name)
    if registry is None:
        logger.warning(f'No worker registry found for queue {queue.name}')
        return
    registry.cleanup()
...
class BaseWorker:
    ...
    queue_class = Queue
    job_class = Job
    workers_registry: Optional[SetRegistry] = None
    ...
    def __init__(
        ...
        if BaseWorker.workers_registry is None:
            BaseWorker.workers_registry = SetRegistry(connection, _KEY_ALL_WORKERS)
        ...
        for queue in queues:
            _QUEUE_WORKERS_REGISTRIES.setdefault(
                queue.name, SetRegistry(connection, _KEY_QUEUE_WORKERS.format(queue.name))
            )
    ...
    def find_by_key(
        ...
        """Returns a Worker instance, based on the naming conventions for naming the internal Redis keys.  Can be used
        to reverse-lookup Workers by their Redis keys.

        :param worker_key: The worker key
        :param connection: Redis connection. Defaults to None.
        :param job_class: The job class if custom class is being used. Defaults to None.
        :param queue_class: The queue class if a custom class is being used. Defaults to None.
        :param serializer: The serializer to use. Defaults to None.

        :raises ValueError: If the key doesn't start with `rq:worker:`, the default worker namespace prefix.
        ...
        if not connection.exists(worker_key):
            BaseWorker.workers_registry.remove(worker_key)
            return None
    ...
    def all(
        ...
        :returns (List[Worker]): A list of workers
        """
        if connection is None and queue is None:
            raise ValueError('Must provide either connection or queue')
        worker_keys = _QUEUE_WORKERS_REGISTRIES[queue.name].all() if queue else BaseWorker.workers_registry.all()
    ...
    def all_keys(cls) -> List[str]:
        ...
        :returns: A list of worker keys
        """
        return BaseWorker.workers_registry.all()

    @classmethod
    def count(cls, queue: Optional['Queue'] = None) -> int:
        """Returns the number of workers by queue or connection.

        :param queue: Queue
        :returns: Number of workers in the queue if queue is provided, otherwise total number of workers
        """

        registry: Optional[SetRegistry] = (_QUEUE_WORKERS_REGISTRIES[queue.name]
                                           if queue is not None else BaseWorker.workers_registry)
        if registry is None:
            return 0
    ...
    def refresh(self):
        ...
        self.hostname = as_str(hostname) if hostname else None
        self.ip_address = as_str(ip_address) if ip_address else None
        self.pid = int(pid) if pid else None
        self.version = as_str(version) if version else None
        self.python_version = as_str(python_version) if python_version else None
        self._state = as_str(state or '?')
        self._job_id = job_id or None
        if last_heartbeat:
            self.last_heartbeat = utcparse(as_str(last_heartbeat))
        else:
            self.last_heartbeat = None
        if birth:
            self.birth_date = utcparse(as_str(birth))
        else:
            self.birth_date = None
        if failed_job_count:
            self.failed_job_count = int(as_str(failed_job_count))
        if successful_job_count:
            self.successful_job_count = int(as_str(successful_job_count))
        if total_working_time:
            self.total_working_time = float(as_str(total_working_time))
        if current_job_working_time:
            self.current_job_working_time = float(as_str(current_job_working_time))

        if queues:
            queues = as_str(queues)
            self.queues = [
                self.queue_class(
    ...
    def clean_registries(self):
        ...
        for queue in self.queues:
            ...
            if queue.acquire_maintenance_lock():
                ...
                clean_queue_worker_registry(queue)
                BaseWorker.workers_registry.cleanup()
    ...
    def queue_keys(self) -> List[str]:
        ...
        :returns: A list of strings with queues keys
        """
    ...
    def pubsub_channel_name(self):
        """Returns the worker's Redis hash key."""
        return WorkerCommand.worker_pubsub_channel(self.name)
    ...
    def work(
        ...
        :param logging_level: Logging level to use. Defaults to "INFO".
        :param date_format: Date Format. Defaults to DEFAULT_LOGGING_DATE_FORMAT.
        :param log_format: Log Format. Defaults to DEFAULT_LOGGING_FORMAT.
        :param max_jobs: Max number of jobs. Defaults to None.
        :param max_idle_time: Max seconds for the worker to be idle. Defaults to None.
        :param with_scheduler: Whether to run the scheduler in a separate process. Defaults to False.
        :param dequeue_strategy: Which strategy to use to dequeue jobs. Defaults to DequeueStrategy.DEFAULT.

        :returns: Will return True if any job was processed, False otherwise.
        """
    ...
    def cleanup_execution(self, job: 'Job', pipeline: 'Pipeline'):
        ...
        if self.execution:
            started_job_registry.remove_execution(self.execution, pipeline=pipeline)
    ...
    def _register_in_queues(self):
        with self.connection.pipeline() as p:
            BaseWorker.workers_registry.add(self.key, p)
            for name in self.queue_names():
                _QUEUE_WORKERS_REGISTRIES[name].add(self.key, p)
            p.execute()
    ...
    def register_birth(self):
        ...
        with self.connection.pipeline() as p:
            ...
            self._register_in_queues()
            p.expire(key, self.worker_ttl + 60)
            p.execute()

    def _unregister_in_queues(self):
        with self.connection.pipeline() as p:
            BaseWorker.workers_registry.remove(self.key, p)
            for name in self.queue_names():
                _QUEUE_WORKERS_REGISTRIES[name].remove(self.key, p)
            p.execute()
    ...
    def shutdown_requested_date(self):
        ...
        if shutdown_requested_timestamp is not None:
            return utcparse(as_str(shutdown_requested_timestamp))
    ...
    def death_date(self):
        ...
        if death_timestamp is not None:
            return utcparse(as_str(death_timestamp))
...
class Worker(BaseWorker):
    ...
    def perform_job(self, job: 'Job', queue: 'Queue') -> bool:
        ...
        if rv is not None:
            self.log.debug('Result: %r', yellow(as_str(str(rv))))
    ...
    def handle_payload(self, message):
        ...
        WorkerCommand.execute_command(self, message)
